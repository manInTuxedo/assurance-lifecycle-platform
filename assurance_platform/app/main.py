...
def ingest_scan(db: Session, filename: str, rows: list[dict]) -> dict:
    """Correlate a VA scan against existing findings & assets."""
    now = datetime.utcnow()
    scan_date = parsers.scan_date_from_filename(filename) or now
    source_type = parsers.detect_source_type(filename)
    ...
    unmapped = new = updated = reappeared = 0
    ...
    for row in rows:
        ...
        # --- Correlation: (IP + Plugin Name + Port + Protocol) ---
        existing = (
    ...
    db.commit()
    return {"records": len(rows), "new": new, "updated": updated,
            "reappeared": reappeared, "unmapped": unmapped}
